	...
	for i := 0; i < len(typ.Fields); i++ {
		...
		case "traceIdHi":
			if typ.Fields[i].Equals(&def.Field{Name: "traceIdHi", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldObjectAllocationOutsideTLAB{Field: &typ.Fields[i], uint64: &res.Temp.TraceIdHi})
			} else {
				res.Fields = append(res.Fields, BindFieldObjectAllocationOutsideTLAB{Field: &typ.Fields[i]}) // skip changed field
			}
		case "traceIdLo":
			if typ.Fields[i].Equals(&def.Field{Name: "traceIdLo", Type: typeMap.T_LONG, ConstantPool: false, Array: false}) {
				res.Fields = append(res.Fields, BindFieldObjectAllocationOutsideTLAB{Field: &typ.Fields[i], uint64: &res.Temp.TraceIdLo})
			} else {
				res.Fields = append(res.Fields, BindFieldObjectAllocationOutsideTLAB{Field: &typ.Fields[i]}) // skip changed field
			}
		default:
