...
func main() {
	...
	write("types/skipper.go", generate(&types.MetadataClass{
...
func TypeForCPoolID(ID types.TypeID) *types.MetadataClass {
	switch ID {
	case T_FRAME_TYPE:
...
func generate(typ *types.MetadataClass, opt options) string {
	res := ""
	...
	if opt.cpool {
		...
		res += fmt.Sprintf("func (this *%sList) Parse(data []byte, bind *%s %s , typeMap *TypeMap) (pos int, err error) {\n", name(typ), bindName(typ), extraBinds)
		receiver = fmt.Sprintf("this.%s[i]", name(typ))
	} else {
		receiver = fmt.Sprintf("this")

		res += fmt.Sprintf("func (this *%s) Parse(data []byte, bind *%s, typeMap *TypeMap) (pos int, err error) {\n", name(typ), bindName(typ))
	}
	_ = receiver
...
func generateBindLoop(typ *types.MetadataClass, bindName string, nestedAllowed bool) string {
...
func fieldsHas(fs []types.Field, tString types.TypeID) bool {
	for _, f := range fs {
		if f.Type == tString {
			return true
		}
...
func getUniqueFields(typ *types.MetadataClass) []types.Field {
	res := make([]types.Field, 0, len(typ.Fields))
...
func getUniqueCpoolFields(typ *types.MetadataClass) []types.Field {
	fs := getUniqueFields(typ)
	res := make([]types.Field, 0, len(fs))
	for _, f := range fs {
		if f.ConstantPool {
			res = append(res, f)
		}
...
func getNonBasicFields(typ *types.MetadataClass) []types.Field {
...
func generateBinding(typ *types.MetadataClass, opt options) string {
	res := ""
	res += fmt.Sprintf("type %s struct {\n", bindName(typ))
	...
	res += fmt.Sprintf("func New%s(typ *MetadataClass, typeMap *TypeMap) *%s {\n", bindName(typ), bindName(typ))
...
func goTypeName(field types.Field) string {
...
func name(typ *types.MetadataClass) string {
	fs := strings.Split(typ.Name, ".")
	s := fs[len(fs)-1]
	return capitalize(s)
}

func bindName(typ *types.MetadataClass) string {
	return "Bind" + name(typ)
}
func bindFieldName(typ *types.MetadataClass) string {
	return "BindField" + name(typ)
}
func refName(typ *types.MetadataClass) string {
	return name(typ) + "Ref"
}

func listName(typ *types.MetadataClass) string {
